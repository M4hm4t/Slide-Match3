...
public class Board : MonoBehaviour
{
    ...
    public void RelocationAll()
    {
        ...
        {
            if (item.TryGetComponent<Dot>(out var dot))
            {
                if (dot.row != dot.transform.position.y)
                {
                    var value = Mathf.RoundToInt(dot.transform.position.y) % height;

                    dot.row = value;

                }
                if (dot.column != dot.transform.position.x)
                {
                    var value = Mathf.RoundToInt(dot.transform.position.x) % width;
                    dot.column = value;
                }
            }
        }
        GameObject[,] tmpArray = new GameObject[width, height];
        foreach (var item in allDots)
        {
            var dot = item.GetComponent<Dot>();
            tmpArray[dot.column, dot.row] = item;
        }
        allDots = tmpArray;
        ...
        {
            var dot = item.GetComponent<Dot>();
            dot.GetComponent<Dot>().isDraging = false;
        }
    }
    ...
    public void Dragging()
    {
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                if (allDots[i, j].TryGetComponent<Dot>(out var dot))
                {
                    dot.GetComponent<Dot>().isDraging = true;
                }
            }
        }
    }
